...
const beritaBws = async () => {
    try {
        ...
        newsItems.each((index, item) => {
            ...
            const thumbnail = $(`.views-row-${i} .views-field-field-foto-berita .field-content img`).attr('src');
            const pubDatel = $(`.views-row-${i} .views-field-created .field-content`).text();
            const title = $(item).find('.views-field-title a').text().trim();
            const description = $(`.views-row-${i} .views-field-body .field-content p`).text();
